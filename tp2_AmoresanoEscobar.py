...
def pixel_art(imagen: Image.Image, tam_bloque: int, niveles_color: int) -> Image.Image:
    ...
    tam_bloque (int): define que tan pixeleada se verá la imagen, el tamaño de cada bloque de píxeles.
    niveles_color (int): Controla la reducción de color de la paleta. Numero menor imprime imagen con menor variedad de colores.
...
def ascii_art(imagen: Image.Image, ancho_ascii: int = 100) -> str:
    ...
    ajusta el tamaño de la imagen, la convierte a escala de grises y convierte 
    ...
    # Ajustamos la proporcion de la imagen para que no se vea distorsionada al convertir a ASCII, ya que los caracteres son mas altos que anchos.
    ...
    # Asociamos cada valor entre 0 y 255 con un carácter ASCII dividiendo el rango por la cantidad de caracteres disponibles.
...
def pedir_metodo()-> str:
    """
    Solicita al usuario ingresar el efecto Pixel Art o ASCII Art.
    Usa un ciclo para validar que la entrada sea correcta, aceptando solo 'pixel' o 'ascii'.
...
def pedir_numero(mensaje: str, tipo: int, valor_default: int) -> int:
    ...
    sino, devuelve un valor predeterminado.
    ...
    Returns:
    int: El número ingresado por el usuario o el valor predeterminado.
    """    
...
def ruta_salida_valida(extension: str) -> str:
    """
    Solicita al usuario una ruta de guardado.    
    ...
    Argumentos:
    extension_requerida (str): La extensión que debe tener el archivo.
    ...
    while True:
        ...
        ruta = input(f"Seleccione la ruta para guardar la imagen procesada ({extension}): ").strip()
        ...
        # Aseguramos que la ruta tenga la extensión correcta, y si no, se anade automáticamente
        ...
        # Verificamos si la ruta ingresada ya existe para evitar sobrescribirlo.
...
def guardar_ascii_art(ascii_art: str, ruta_salida: str): 
    """
    Guarda la cadena de texto del arte ASCII en un archivo de texto (.txt).

    Argumentos:
    ascii_art (str): La cadena de texto del arte ASCII.
    ruta_salida (str): La ruta de destino donde se creará el archivo.
    """
...
def metodo_pixel(imagen: Image.Image):
    """
    Controla el proceso para transformar y guardar la imagen en modo Pixel Art.
    Se encarga de pedir al usuario el tamaño del bloque y los niveles de color, 
    aplicar el efecto sobre la cuadrícula de la imagen y gestionar el guardado 
    del archivo final en formato PNG.

    Argumentos:
    imagen (Image.Image): imagen ya cargada.
    """
    # Se solicita al usuario los parametros necesarios para el efecto Pixel Art
    tam_bloque = pedir_numero("Ingrese el tamaño del bloque", 1, 10)
    niveles_color = pedir_numero("Ingrese los niveles de color", 2, 4)

    # Procesamos la imagen con los parametros obtenidos
    resultado = pixel_art(imagen, tam_bloque, niveles_color)

    # Guardamos la imagen editada. Se asegura que la ruta sea válida y que el archivo se guarde con extensión .png
...
def metodo_ascii(imagen: Image.Image):
    """
    Controla el proceso para transformar y guardar la imagen en modo ASCII Art.
    Se encarga de pedir al usuario el ancho en caracteres del resultado ASCII,
    aplicar el efecto sobre la cuadrícula de la imagen y gestionar el guardado
    del archivo final en formato de texto (.txt).
    Argumentos:
    imagen (Image.Image): imagen ya cargada.
    """
    
    # Se solicita al usuario el ancho en caracteres del resultado ASCII
    ancho_ascii = pedir_numero("Ingrese el ancho ASCII [1-1000]", 3, 100)

    # Procesamos la imagen con el parametro obtenido
    resultado = ascii_art(imagen, ancho_ascii)
    
    # Guardamos el arte ASCII generado. Se asegura que la ruta sea válida y que el archivo se guarde con extensión .txt
...
def main():
    ...
    Solicita al usuario el archivo de la imagen a procesar, 
    ejecuta el metodo de transformación seleccionado y 
    maneja cualquier error inesperado durante la ejecución del programa.
    """
    try:
        # Solicitud de la ruta de la imagen al usuario y validación de su existencia
    ...
    # Captura de cualquier error inesperado durante la ejecución del programa
    except Exception as e:
